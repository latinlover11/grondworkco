...
# Standardize navigation headers across all HTML files to match index.html format
...
for file in *.html blog/*.html case-studies/*.html; do
  if [[ -f "$file" ]]; then
    ...
    # Define the standard header from index.html, but with paths adjusted by PREFIX
    # Note: index.html uses paths like "index.html#services", "work.html", etc.
    # We need to prepend PREFIX to these, but note that for the root files, PREFIX is empty.
    # Also, we need to handle the active link.
    
    # Determine active item based on file name
...
$MOBILE_LINKS
    ...
    # Find the lines for <header> and </header>
    HEADER_START=$(grep -n "<header>" "$file" | head -1 | cut -d: -f1)
    HEADER_END=$(grep -n "</header>" "$file" | head -1 | cut -d: -f1)
    
    if [[ -z "$HEADER_START" || -z "$HEADER_END" ]]; then
      echo "Warning: Could not find header tags in $file"
      continue
    fi
    ...
    # Copy lines before header
    sed -n "1,$((HEADER_START-1))p" "$file" > "$TMP_FILE"
    # Add new header
    echo "$NEW_HEADER" >> "$TMP_FILE"
    # Copy lines after header
    sed -n "$((HEADER_END+1)),\$p" "$file" >> "$TMP_FILE"
