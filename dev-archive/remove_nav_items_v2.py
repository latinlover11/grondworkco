...
def remove_process_contact_from_nav():
    ...
    for filepath in html_files:
        ...
        for link in soup.select('header .nav-links a[href]'):
            href = link['href']
            # Check if this is a Process or Contact link (including relative paths)
            if any(x in href for x in ['#process', 'process.html', '/process']) or \
               any(x in href for x in ['contact.html', '/contact', 'contact']):
