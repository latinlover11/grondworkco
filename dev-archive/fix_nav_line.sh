...
$MOBILE_LINKS
    ...
    HEADER_START=$(grep -n "<header>" "${file}.bak" | head -1 | cut -d: -f1)
    HEADER_END=$(grep -n "</header>" "${file}.bak" | head -1 | cut -d: -f1)
    ...
    # Create the new file
    {
      ...
      sed -n "1,$((HEADER_START-1))p" "${file}.bak"
      # Print the new header
      echo "$NEW_HEADER"
      # Print lines after the header
      sed -n "$((HEADER_END+1)),\$p" "${file}.bak"
    } > "$file"
