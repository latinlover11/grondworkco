...
def resolve_path(base_dir, file_path, link_href):
    ...
    if link_href.startswith('/'):
        # Root-relative: resolve from base_dir (document root)
        absolute_link_path = os.path.normpath(os.path.join(base_dir, link_href.lstrip('/')))
    else:
        # Relative to the file's directory
        # Get the directory containing the file
        file_dir = os.path.dirname(file_path)
        # Resolve the link relative to the file's directory
        # This handles ../, ./, and relative paths correctly
...
def check_file(filepath):
    ...
    for img in soup.find_all('img'):
        src = img.get('src')
        if not src:
            # Skip lightbox placeholder images (intentionally empty src)
        ...
        if src.startswith('data:') or src.startswith('http://') or src.startswith('https://'):
            continue
        # Resolve relative to the file's directory (how browsers do it)
        img_path = resolve_path(BASE_DIR, filepath, src)
